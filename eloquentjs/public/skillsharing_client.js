/**
 * Created by zhangyue on 2016/12/8.
 */
...
function instantianteTemplate(name,values){
...
function drawTalk(talk){
    var node = instantianteTemplate("talk",talk);
    var comments = node.querySelector(".comments");
    talk.comments.forEach(function(comment){
        comments.appendChild(
            instantianteTemplate("comment",comment)
        );
    });
...
function waitForChanges(){
    request({pathname:"talks?changeSince="+lastServerTime},function(error,response){
        if(error){
